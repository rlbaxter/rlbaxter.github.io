<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Product Bootcamp - FAQs</title>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<link rel="stylesheet" href="styles/style.css">
</head>
<body>
	<div class="headers">
		<header class="primary-header">
			<nav class="primary-nav">
				<ul>
					<!--Mobile Order: Actual Order. Desktop Order: Class Names. Bless you Flexbox-->
					<li class="order-3"><a href="product-bootcamp.html" class="bootcamp active">Product Bootcamp</a></li>
					<li class="order-2"><a href="#" class="case-study">Case Study Conference</a></li>
					<li class="order-4"><a href="custom-training.html" class="custom-training">Custom Onsite Training</a></li>
					<li class="order-5"><a href="#" class="magazine">Magazine</a></li>
					<li class="order-1"><a href="index.html" class="home">Home</a></li>
				</ul>
			</nav>
			<nav class="social-nav">
				<ul>
					<li><a class="twitter" href="https://twitter.com/front">Twitter</a></li>
					<li><a class="linkedin" href="https://www.linkedin.com/company-beta/4873375/?pathWildcard=4873375">LinkedIn</a></li>
					<li><a class="instagram" href="https://www.instagram.com/frontutah/">Instagram</a></li>
				</ul>
			</nav>
		</header>
		<header class="section-header">
			<nav class="section-nav">
				<div class="section-nav-head">
					<h3 class="logo">Front</h3>
					<button class="menu-toggle"></button>
				</div>
				<ul>
					<li><a href="courses-ux.html">Courses</a></li>
					<li><a href="schedule.html">Schedule</a></li>
					<li><a href="instructors.html">Instructors</a></li>
					<li><a href="venue.html">Venue</a></li>
					<li><a href="faqs.html">FAQs</a></li>
					<li><a href="why-attend.html">Why Attend</a></li>
					<li><a href="registration.html" class="button">Register</a></li>
				</ul>
			</nav>
		</header>
	</div>
	<main class="product-bootcamp-page">
		<section class="hero">
			<h1>FAQs</h1>
		</section>
		<section class="faqs">
				<h2>FAQs</h2>
				<p class="about-faqs">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
				<ul class="faq-categories">
					<li class="faq-category">
						<h3>Registration information</h3>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
					</li>
					<li class="faq-category">
						<h3>Registration information</h3>
						<dl class="accordion">
							<dt class="accordion-title question">This is a slightly more verbose question that will require some line wrapping possibly, or maybe not, who knows?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
					</li>
					<li class="faq-category">
						<h3>Registration information</h3>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
					</li>
					<li class="faq-category">
						<h3>Registration information</h3>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
						<dl class="accordion">
							<dt class="accordion-title question">This is a question?</dt>
							<dd class="accordion-content answer">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</dd>
						</dl>
					</li>
				</ul>
		</section>
		<section class="join-us">
			<h2>Join us at the Front!</h2>
			<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.</p>
			<p><a href="" class="button">Register</a></p>
		</section>
	</main>
	<footer>
		<nav class="social-nav">
			<ul>
				<li><a class="twitter" href="https://twitter.com/front">Twitter</a></li>
				<li><a class="linkedin" href="https://www.linkedin.com/company-beta/4873375/?pathWildcard=4873375">LinkedIn</a></li>
				<li><a class="instagram" href="https://www.instagram.com/frontutah/">Instagram</a></li>
			</ul>
		</nav>
		<nav class="main-footer-nav">
			<ul>
				<li><a href="#">Contact Us</a></li>
				<li><a href="#">Terms &amp; Conditions</a></li>
				<li><a href="#">Code of Conduct</a></li>
				<li><a href="#">Speaker Guide</a></li>
			</ul>
		</nav>
		<p>Join us at the</p>
		<h3 class="logo">Front</h3>
		<p>&copy; Front, LLC. All rights reserved.</p>
	</footer>
	<script src="scripts/jquery-3.2.1.min.js"></script>
	<script src="scripts/ui.js"></script>
</body>
</html>